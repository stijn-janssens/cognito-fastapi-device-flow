...
from app.device_flow.model import DeviceFlow as DeviceFlowModel
from app.device_flow.schema import DeviceFlow, DeviceFlowStatus
...
class DeviceFlowService:
    ...
    def fetch_jwt_tokens(
        ...
        if device_flow.status == "expired":
            raise ValueError("The device code has already expired")
        ...
        if utc_now() > device_flow.max_expiry:
            ...
            db.commit()
            raise ValueError("The device code has expired")
    ...
    def deny_device_flow(self, device_code: UUID, db: Session):
