...
from app.device_flow.service import DeviceFlowService, verify_auth_header
...
def deny_device_flow(device_code: UUID, db: Session = Depends(get_db_session)):
    return DeviceFlowService().deny_device_flow(device_code, db)


@router.get("/allow", include_in_schema=False)
def allow_device_flow(
    client_id: str, device_code: UUID, db: Session = Depends(get_db_session)
):
